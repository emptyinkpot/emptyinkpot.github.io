...
## 10. 对标成熟博客与社交平台的升级计划（2026-05-10）

### 10.1 目的

当前 MyBlog 已经不是基础博客壳层，而是一个内容运行面。但它距离成熟内容平台仍有差距，主要体现在：

- 内容组织还偏静态站思维，缺少更强的知识型入口
- 前台交互已有运行层概念，但统一性和产品感还不够
- 社交分发、订阅、关注、会员、推荐、反馈闭环仍薄
- 站点的“平台感”还没有完全压过“博客模板感”

本阶段不是重做底层，而是借鉴成熟系统的可复刻部分，把 MyBlog 收敛成更成熟的内容平台。

### 10.2 参考对象

优先参考以下成熟方案：

- **Ghost**：成熟博客/出版平台，重点借鉴发布后台、内容组织、会员/订阅、主题分层和前后端解耦思路
- **Quartz**：知识型博客 / 数字花园，重点借鉴 Markdown 驱动、双向链接、目录感、知识组织方式
- **Astro 官方博客模板**：工程基线和内容站规范，重点借鉴内容集合、静态发布和组件分层
- **WriteFreely / Mastodon**：轻社交和联邦分发思路，重点借鉴时间线、公开写作和社交传播，而不是照搬实现
...
1. 对齐内容结构
   - 明确 `posts` / `notes` / `projects` / `pages` 的职责
   - 给专题、系列、标签、项目入口建立统一信息层
2. 对齐首页编排
   - 强化主推内容、最新内容、系列入口、项目入口、短内容入口
   - 降低纯文章列表感
3. 对齐阅读体验
   - 强化目录、上下文阅读、相关文章、反馈入口
   - 保留 Drawer / Reader 的运行层连续性
4. 对齐社交分发
   - 把外部分享、订阅、公开更新、轻反馈做成稳定层
   - 参考社交平台的时间线感，而不是单纯文章列表
5. 对齐工程基线
   - 保持 Astro 作为站点装配层
   - 继续用现有 contracts / philosophy / topology 体系约束前端

### 10.5 验收标准

- 站点首页明显更像内容平台，而不是模板博客
- 内容组织能自然支撑长期扩展，不需要频繁改路由体系
- 前台阅读和浏览连续性更强，交互更统一
- 文档能清楚说明哪些是借鉴、哪些是自研、哪些刻意不做
- 后续新功能能在现有结构上继续增长，而不是重新推倒

### 10.6 下一步执行顺序

1. 先补充一份 `docs/architecture/blog-platform-reference-map.md`，列出参考系统、借鉴点和不采用项
2. 再把首页、内容页、系列页、项目页的信息架构收拢成一个小步改造清单
3. 之后再进入前台组件和布局改造
4. 最后补验证、截图和上线说明
