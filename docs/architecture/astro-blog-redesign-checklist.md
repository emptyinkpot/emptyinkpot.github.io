...
关联详细方案：`docs/architecture/astro-blog-redesign-plan.md`
...
- [x] 新建 `docs/architecture/astro-blog-redesign-plan.md`
- [x] 新建 `docs/architecture/astro-blog-redesign-checklist.md`
- [x] 在仓库中建立 `apps/web/`（由 `site-v2/` 过渡）
...
- [x] 为 `apps/web` 接入 Sitemap
- [x] 为 `apps/web` 接入 RSS
- [x] 为 `apps/web` 接入 `robots.txt`
...
- [x] 迁移剩余 3 篇 Hexo 文章到 `apps/web`，补齐 `description / series / 内部链接`
- [x] 验证迁移后的首页卡片、系列页与文章详情页路由

## 21. 下一批优先动作

- [x] 清理剩余公开文档与内容中的双轨表述
- [x] 梳理旧 Hexo 内容到 Astro 内容源的迁移状态表
- [x] 迁移最后一篇待处理文章到 Astro 现行内容层并改写为单站点口径
- [x] 检查旧链接兼容与重定向策略，并为历史 slug 增加兼容入口
- [x] 将 `npm run check` 接入 CI 与发布前检查流程
...
- [x] 建立内容治理校验脚本：`npm run check:content`
- [x] 将公开维护日志与 `apps/web` 上线后的线上站点同步策略写清楚
- [x] 将验证记录标准化为固定命令集合：`npm run check`、`npm run build`
