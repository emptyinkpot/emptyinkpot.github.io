...
- `tools/import-contentbase-projection.mjs`：导入 ContentBase `public-projection.v1` 包并复用 MyBlog 运行时索引构建器生成可验收投影
...
- 导入 ContentBase 投影包时使用 `npm run import:contentbase-projection -- --projection <ContentBase public-projection.v1 directory>`
