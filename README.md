...
authoringTruthRoot: OpenList /夸克网盘/obsidian/data/docs
publishingTruthRoot: E:\My Project\MyBlog\apps\web\src\content\posts
...
docsEntrypoints:
  frontendRuntimeAudit: E:\My Project\MyBlog\docs\frontend-runtime-audit.md
  frontendRuntimeConvergence: E:\My Project\MyBlog\docs\frontend-runtime-convergence.md
  runtimeExperienceLayer: E:\My Project\MyBlog\docs\runtime-experience-layer.md
...
- 写作母库真源：OpenList 路径 `/夸克网盘/obsidian/data/docs`
- 公开文章发布真源：`E:\My Project\MyBlog\apps\web\src\content\posts`
...
```bash
npm run build
tar -czf myblog-web.tgz -C apps/web/dist .
scp myblog-web.tgz ubuntu@124.220.233.126:/tmp/
ssh ubuntu@124.220.233.126 'set -e; tmp=/tmp/myblog-web; sudo rm -rf "$tmp"; sudo mkdir -p "$tmp"; sudo tar -xzf /tmp/myblog-web.tgz -C "$tmp"; sudo rsync -a --delete "$tmp"/ /srv/myblog/site/; sudo chown -R www-data:www-data /srv/myblog/site; sudo rm -rf "$tmp"'
```
...
- 书架新增书籍来自 OpenList 路径 `/夸克网盘/obsidian/data/docs/books/original` 的实时列表；封面来自 `public-data/openlist-covers/manifest.json` 和已落盘真实封面缓存，不在访客访问时临时解析。
...
- Vault File Truth / Obsidian 文件真源是 OpenList 暴露的完整 Vault 路径 `/夸克网盘/obsidian/data`；`docs`、`image`、`.obsidian`、`canvas`、PDF 和附件必须作为同一个 Vault 同步，而不是由博客分别手工镜像。
- Authoring Truth / 写作母库真源是 Vault 内的 `/夸克网盘/obsidian/data/docs` 子树；它承载长期原稿、私人笔记、资料、草稿、附件引用和未发布知识，不直接等同公开博客。
- Publishing Truth / 公开发布真源是 `apps/web/src/content/posts/`；首页、文章页、RSS、标签、分类、系列、搜索索引和知识图谱只能读取已进入 Astro content collection 的发布稿。
- Content Root Truth / 站点内容根是 `apps/web/src/content/`，正式集合边界是 `posts`、`notes`、`projects`、`pages`。
- OpenList 是 File Truth / 文件层，不是 CMS；它可以承载 Obsidian Vault、书籍、图片、视频和附件，但不能直接替代 Astro 发布集合。Vault 同步必须优先走成熟同步层：Obsidian Remotely Save -> OpenList WebDAV / S3-compatible endpoint -> OpenList/COS，而不是 MyBlog 自己上传 `data/image` 或重造双向同步协议。大文件优先进入 OpenList 挂载的对象存储，不继续挤占服务器 root disk。
- MySQL 是 Runtime Truth / 运行时状态层，不承载文章正文真源；它只保存阅读记忆、高亮、批注、关系、历史、贴纸和后续 Graph runtime。
- Obsidian 到网站之间必须存在 Publish Pipeline：scan -> filter `published: true` -> normalize -> copy/write `apps/web/src/content/posts/` -> Astro build -> Pagefind/search/Graph。禁止把 `/夸克网盘/obsidian/data/docs` 整个目录直接暴露为公开站点内容。
...
- Obsidian Vault 是认知空间；MyBlog 是发布空间。Obsidian Markdown 中的 `[[双链]]`、附件路径、私有笔记、未发布草稿和碎片笔记必须经过 Normalize Layer 后才允许进入 `posts`。
...
- 不要把 OpenList 的 `/夸克网盘/obsidian/data/docs` 直接挂成公开文章真源；它是上游写作母库，必须经过 Publish Pipeline 才能进入 Astro collection。
...
- `/夸克网盘/obsidian/data/docs` 是上游写作母库真源。
- `apps/web/src/content/` 是站点内容根真源。
- `apps/web/src/content/posts/` 是公开文章发布真源。
...
- 当前上游写作母库真源：OpenList `/夸克网盘/obsidian/data/docs`
...
Obsidian local vault
  ↕ Remotely Save
OpenList WebDAV / S3-compatible endpoint
  ↕
OpenList / COS file truth
  ↓
OpenList / Obsidian docs subtree
  ↓
...
- Obsidian / OpenList 路径 `/夸克网盘/obsidian/data` 是完整 Vault File Truth，必须包含 `docs`、`image`、`.obsidian`、canvas、PDF 和附件。Remotely Save / WebDAV 负责这一层的双向同步，MyBlog 不再手工镜像 `data/image`。
- Obsidian / OpenList 路径 `/夸克网盘/obsidian/data/docs` 是 Authoring Truth 子树，负责写作、草稿、私人知识和附件组织。
- `apps/web/src/content/posts/` 是 Publishing Truth，负责公开文章、SEO、RSS、标签、分类、搜索和 Graph 输入。
- Publish Pipeline 是唯一允许从 Authoring Truth 进入 Publishing Truth 的通道；它不是简单文件夹复制，而是 Content Compile。
- 初始发布条件建议使用 frontmatter：`published: true`、`draft: false`、`slug`、`title`、`date`、`summary`、`tags`、`categories`。
- 未标记发布、缺必要 frontmatter、引用私有附件、存在无法解析 Obsidian 双链的文件不得进入 `posts`。
...
```bash
# dry-run：只扫描并报告，不写入 posts
npm run publish:vault -- --vault "E:/My Project/content-vault"

# 使用环境变量指定 Vault
MYBLOG_VAULT_ROOT="E:/My Project/content-vault" npm run publish:vault

# 写入 apps/web/src/content/posts/
npm run publish:vault:write -- --vault "E:/My Project/content-vault"
```

脚本合同：

- 脚本位置：`tools/publish-vault-docs.mjs`。
- 默认只 dry-run；只有 `--write` 或 `npm run publish:vault:write` 才写入 `apps/web/src/content/posts/`。
- 只发布带 frontmatter 且 `published: true`、`draft` 非 `true` 的 Markdown / MDX。
- 输出文件名使用稳定 `slug`，写入 `apps/web/src/content/posts/<slug>.md`。
...
- 自检脚本 `tools/validate-vault-publish-pipeline.mjs` 已接入 `npm run check`，会创建临时 Vault 验证 dry-run 不写文件、`published: true` 过滤、`drafts/` 跳过、folderTags 合并、Obsidian 双链转换和 `--write` 输出清理。
...
- `OpenList + 腾讯云 COS` 是当前文件真源 / 大文件存储层。COS bucket 是 `myblog-media-1410041307`，region 是 `ap-shanghai`，OpenList 挂载点是 `/腾讯云COS`，已验证 `_verify/openlist-cos.txt` 可写入并可从 bucket 侧看到。
...
| --- | --- | --- | --- |
| 文件真源 / 大文件 | OpenList + Tencent COS | 已挂载并验证 COS 写入 | 通过同域 `/openlist/` 和 `/api/openlist/*` 展示 / 读取 |
...
MyBlog Runtime API / Object Layer Glue
  ↓
Web / PWA-TWA / Android / Search / CLI / AI Agent
```
...
| Remotely Save | Vault Sync | 成熟 WebDAV / S3 同步，不由 MyBlog 自写双向 sync | target workflow |
| OpenList + COS | Blob / WebDAV / File Truth | EPUB、PDF、图片、视频、Vault 文件和归档文件 | active |
...
- 书架不再维护静态书籍种子；OpenList 索引与 `/夸克网盘/obsidian/data/docs/books/original` 实时列表是书籍存在性唯一来源。`apps/web/src/data/books.ts` 只能作为 metadata overlay，不能 resurrect OpenList 不存在的书。
...
- WebDAV 入口由 OpenList 自身提供，路由是 `/openlist/dav/`；Remotely Save 配置时应把整个 Obsidian Vault 同步到 `/夸克网盘/obsidian/data` 或未来迁移后的 `/腾讯云COS/obsidian/data`，不要只同步 `data/docs`。WebDAV 用户必须具备 read/write/manage 权限，凭据只保存在 Obsidian 插件配置或服务器环境，不进入仓库、README、日志或前端 bundle。
...
- OpenList Index 是文件目录快照，不是 Vault 同步器。若线上 `/夸克网盘/obsidian/data` 没有 `image` 目录，根因应先从 Obsidian Remotely Save / WebDAV / OpenList 挂载同步层排查，不能通过 MyBlog 的 index rebuild、visual manifest 或一次性 API upload 伪装修复。
- 服务器磁盘策略：`/dev/vda2` 当前是 40GB root disk，Lighthouse 不支持直接 `ResizeDisks` 扩当前系统盘；新增大文件不应落到 `/srv/myblog/public-data/openlist-files` 这类本地缓存作为长期存储。长期原件进入 COS 或网盘，访客 reader 本地缓存只保留已索引、可淘汰的派生数据。
...
- Visual System 的成熟组合服务栈是 `OpenList + COS` 做文件真源、`Immich` 做 AI media runtime、`Directus` 做 metadata overlay、`Meilisearch` 做 search runtime、`Astro/MyBlog` 做展示壳。当前只有 OpenList + COS 已验证，Immich 是 skeleton 未启动，Directus / Meilisearch 仍是 target runtime；源码和 UI 不得把未部署服务写成已可用能力。
...
- Obsidian Vault 内的本地素材不是 Visual bookmark mirror。`E:\My Project\Atramenti-Console\codex\plugins\obsidian\data\image` 这类 Vault attachment 应通过 Remotely Save 同步到 OpenList WebDAV 对应 Vault 路径，再由 MyBlog 按 OpenList 文件索引或 Publish Pipeline 消费；禁止用 Pinterest/Pixiv bookmark sync、visual-manifest 或一次性 API upload 来承担 Vault 双向同步。
...
- 站点代码统一放在 `apps/web/`
- 工程文档统一放在 `docs/`
- 原稿写作优先落在 Obsidian Vault：完整 Vault 文件真源是 OpenList `/夸克网盘/obsidian/data`，写作母库子树是 `/夸克网盘/obsidian/data/docs`
- Obsidian Vault 的图片、PDF、canvas 和附件由 Remotely Save + OpenList WebDAV/S3 同步；不要把这些文件手工复制到 `apps/web/public/images/` 或通过 MyBlog API 单向上传到 OpenList
- 公开发布内容统一通过 Publish Pipeline 落在 `apps/web/src/content/posts/`
- 工具脚本统一放在 `tools/`
