...
- [OpenClaw Extensions 模块化架构总纲：从平台壳到模块闭环](/posts/openclaw-extensions-architecture-blueprint/)
...
4. 如果项目根 `control-ui-custom` 当前是 `Junction` 或其他链接壳，必须先拆掉链接关系，再恢复为真实实体目录
5. 删除镜像副本之前，必须先搞清楚项目根目录是否依赖该镜像副本存活
...
| 当前来源 | 目标归属 | 处理原则 |
| --- | --- | --- |
| `plugins/novel-manager/**` | `novel-manager/**` | 整体归位，`plugin` 变薄壳 |
...
| `plugins/automation-hub/**` | `automation-hub/**` | 前后端合并入模块内 |
...
7. 在 `extensions` 根下建立 workspace 级 `package.json`
8. 建立 `tsconfig.base.json`
9. 建立统一路径别名规则，只允许：
   - `@extensions/kernel/*`
   - `@extensions/<module>`
   - `@extensions/<module>/contracts`
10. 建立 `.eslintrc.cjs` 的 import boundary 规则
...
20. 把这些接口迁回 `experience-manager/backend/routes`
...
39. 把小说公开 DTO、请求响应结构、事件定义放到 `novel-manager/contracts`
...
58. 把文件系统、监听器、规则引擎分别收口到 `core` 或 `backend/services`
...
62. 后端路由统一为 `/api/ai-hub/*`
63. 本地脚本或启动器放到模块内 `scripts`，不再散落顶层
...
66. 将 `memory-lancedb-pro` 规范为：
   ...
   - `mcp`
   - `cli`
67. 将 `feishu-openclaw-plugin` 重命名并迁入 `feishu-openclaw`
68. 删除旧 `plugins/qmd`、`plugins/memory-lancedb-pro`、`plugins/feishu-openclaw-plugin` 目录
...
71. 导航从 `module.json` 动态生成，不再手改共享导航 HTML
72. 校验所有路由前缀无冲突
73. 校验所有页面前缀无冲突
...
95. 验证源码树中已无运行态目录
96. 验证顶层只剩平行模块、`kernel` 和 workspace 文件
...
4. 真正的模块化不是“把代码挪一下”，而是把页面、路由、核心、契约、插件入口全部收回模块内部
...
> **把 OpenClaw 从“顶层全局业务杂物间 + 镜像副本 + 手工导航 + 路由越权”的混合工程，重构成“平台克制、模块闭环、声明驱动、边界可检查、运行态外置”的模块化单体平台，并让这套规则可以被人和 AI 一起稳定执行。**
