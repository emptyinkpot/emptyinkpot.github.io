...
- 写现行内容时，只进入 `site-v2/src/content/`。
- 调整界面时，只进入 `site-v2/src/components/`、`site-v2/src/layouts/`、`site-v2/src/pages/`、`site-v2/src/styles/`。
...
3. 明确 `site-v2/` 将承担最终线上站点职责。

如果从零开始，初始化命令类似：

```bash
npm create astro@latest site-v2
cd site-v2
npm install
```
...
- 将现行文章迁入 `site-v2/src/content/posts/`。
